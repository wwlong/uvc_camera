...
int init_camera(int fd)
{
    ...
    {
        fprintf(stderr, "VIDIOC_G_FMT get err\n");
        exit(-1);
        close(fd);
    }
    printf("tv_fmt.type : %d\ntv_fmt.fmt.pix.width %d\ntv_fmt.fmt.pix.height%d\ntv_fmt.fmt.pix.pixelformat:%d\nv_fmt.fmt.pix.field:%d\n", tv_fmt.type, tv_fmt.fmt.pix.width, tv_fmt.fmt.pix.height, tv_fmt.fmt.pix.pixelformat, tv_fmt.fmt.pix.field);
